...
⚠ **`dbg_order.py` 的「en 在前」只统计双侧 pair**；单侧段另立「仅英文」列。
**别把「仅英文」当顺序问题**（§6.22 ② 的教训：1031 里 1030 是假警报）。
`--list` 看真倒置、`--listone` 看单侧段。
...
>
> ★ **最新成品（`_V=15`，题注修复后，2026-09-18 14:0x）**：
> `diag/prob_p4/Probability Theory the Logic of Science_双语.{epub,html}`
> —— **以这个为准**。相比 `prob_p3`：`dbg_qa` **40→39 类**（`Table 6.1.The effect o…`
> 那条误报消失）、题注段 **18→35**（EN 13 + ZH 22，此前 EN 侧 0）、
> **仅英文段 37→27**（净 −10，题注配对改善）。其余门禁与 p3 逐项一致：
> `bookscan` **0/0/0** · 真·倒置 **0/6109** · `eqcheck` exit 0 · 段落对 **6136** 不变。
> 见 §6.23。
>
> **`prob_p3/`**（fm-title 修复后，13:51）保留作对照：`dbg_bookscan` **首次全 0**，
> `dbg_qa` 103→31（40 类）。
>
> `_V` 当前 **15**（§6.22 的 fm-title 解析 fix 时 14→15；§6.23 的题注修复**在渲染层 + pipeline，
> 不碰 `fastcache` 用户，故第 15 版不动**，理由见 §6.23 ③）。
>
> ⚠ **`dbg_order.py` 的判据已修**（§6.22 ②）：「en 在前」列现在**只统计双侧 pair**，
> 单侧段另立「仅英文」列。**旧版把「缺中文」混进「顺序倒置」，1031 里 1030 是假警报。**
> ⚠⚠ **`dbg_order.py` 已再加第二道防线**（§6.23 ④）：无侧别 token 的裸 `caption`
> 类段，按 `zh` 计。**新增段落类型时必须同时带侧别 token**，否则这把尺子会再次误报。
>
> 下面这段是更早的存档，**已被上表取代**。
...
> ★ **2026-09-18 14:0x 更新（§6.23 题注修复）**：fm-title 已落库（`_V=15`），
> **题注三连缺陷已修**，最新成品 `diag/prob_p4/`（`bookscan` 0/0/0 · qa 39 类 ·
> 倒置 0/6109 · 仅英文 27）。用户定调「不排半价时段、一个一个干完」，
> 所以下面几项**直接排着做，不再攒到凌晨**。
>
> **当前队列**：
> 1. ✅ fm-title 卷头标题（§6.22）—— **已完成并验收**
> 2. ✅ 题注识别三连缺陷（§6.23）—— **已完成并验收**（`534a365`）
> 3. **`build.py:1424` 的 `if res.notes:`**（§6.7 末尾）—— 英文注释兜底被挡在门内，
>    改 `if res.notes or _ids:` 会一次多出近千条英文原注，**需先估影响面**
> 4. **27 个「仅英文段」**（§6.23 ⑥）—— 真·缺中文，分布在 ch5/7/8/10/11/12/13/14/
>    17/18/19/20/22/23/26/28/29，多数是表格标题 + 公式引导语，性质需逐条判；
>    **题注配对错链**（ZH 题注在表前 / EN 在图后）是其中一部分，锚点化候选待估
> 5. 早期遗留：1.34 补译重复 · 2.13 漏译 · 自渲染公式残留
