...
>
> ★ **新成品（`_V=15`，fm-title 修复后）**：
> `diag/prob_p3/Probability Theory the Logic of Science_双语.{epub,html}`
> （23.4MB / 29.3MB，13:51）。**`dbg_bookscan` 首次全 0**，`dbg_qa` 103→31。
> 上表 bookscan/qa/顺序列即取自这个新成品；段落对等 stdout 指标与旧成品相同。
>
> `_V` 当前 **15**（§6.22 的 fm-title 解析 fix 时 14→15）。
>
...
> ★ **2026-09-18 13:5x 更新（§6.22）**：fm-title 已落库（`_V=15`），
> `dbg_bookscan` **首次全 0**，`dbg_qa` 103→31。用户定调「不排半价时段、
> 一个一个干完」，所以下面几项**直接排着做，不再攒到凌晨**。
>
> **当前队列**：
> 1. ✅ fm-title 卷头标题（§6.22）—— **已完成并验收**
> 2. **`build.py:1424` 的 `if res.notes:`**（§6.7 末尾）—— 英文注释兜底被挡在门内，
>    改 `if res.notes or _ids:` 会一次多出近千条英文原注，**需先估影响面**
> 3. **37 个「仅英文段」**（§6.22 ②）—— 真·缺中文，分布在 ch5/7/8/10/11/12/13/14/
>    17/18/19/20/22/23/26/28/29，多数是表格标题 + 公式引导语，性质需逐条判
> 4. 早期遗留：1.34 补译重复 · 2.13 漏译 · 自渲染公式残留
