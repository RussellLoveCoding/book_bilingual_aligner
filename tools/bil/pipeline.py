...
class SectionResult:
    ...
    # v4（2026-09-19 §6.41）：提示框裸标签（`<h6>Note</h6>` 降级而来的
    # `box_label` 块）。**不参与对齐**（中译本把提示框摊平成了普通段，
    # 没有这个标签），但内容要照原版渲染在提示框内顶部。渲染层按 `box`
    # 类型把它贴回同类型的提示框 pair；取不完的兜底单独渲染。
    # ⚠ 必须在这里显式带过去 —— `align.Section.box_labels` 与
    # `SectionResult` 是**两个类**，不带就静默丢内容（实测 ch4 的 20 个
    # 标签全部消失）。
...
def cut_notes(zh_blocks, n_notes):
    ...
    paras = [b for b in zh_blocks if b.is_para and not _is_visual(b)]
    ...
    kept = [b for b in zh_blocks
            if b.type == "heading" or b.type == "box_label"
            or _is_visual(b) or id(b) in keep]
...
def _split_wide_pairs(pairs, en_paras=None, zh_paras=None,
    ...
    ➕ 2026-09-19 修（用户截图「ch6 开头错配，典中典」）：原判据要求
    **`min(n,m) >= 2`**，于是 **`2en:1zh`**（`min=1`）**整类漏掉**——
    两段英文塞进一对、只有一段中文，第二段英文成了「哑巴对」，**后面
    所有中文整体错位一格**（用户截图里 ch6 第一对就是这个）。实测 ml 有
    **3 处**（pair 0/42/45）。
    现补一条窄支路：**`n>=2` 且 `m==1`（或反过来）**时，若「多出来的那侧」
    的段与「唯一那侧」长度比明显不合（说明它俩不是一个段），就拆成
    `1:1 + 1:0`。判据同样要求**逐对长度比合法**、**代码块/列表项不拆**
    ——即复用下面的 ①② 校验，只是把 `min>=2` 放宽到 `min>=1`。
    ⚠ 只处理「一侧为 1」的情形；两侧都 ≥2 走原逻辑，不叠加、
      保证 diff 面最小（避免把已调好的宽组行为改坏）。
    """
    out = []
    for p in pairs:
        n, m = len(p.en or []), len(p.zh or [])
        if not (n and m) or n == m:
            out.append(p)
            continue
        # 2en:1zh / 1en:2zh：一侧恰为 1、另一侧 ≥2 → 单独走「抽哑巴」支路
        if min(n, m) == 1 and max(n, m) >= 2:
            out.extend(_depair_singleton(p, en_paras, zh_paras, r_lo, r_hi))
            continue
        if min(n, m) < 2:
            out.append(p)
            continue
...
def _depair_singleton(p, en_paras, zh_paras, r_lo: float, r_hi: float):
    """一侧恰 1 段、另一侧 ≥2 段 → 尝试把「多出来的段」摘成哑巴对。

    用户截图（2026-09-19，ch6 开头）：`pair[0]` 里 en=[段A, 段B]、zh=[段a]，
    段B（`Decision trees are also the fundamental…`）**在中文版里有对应译文**，
    只是被 DP 塞进了上一对 → 它在成品里没中文，且**后面所有中文整体左偏一格**。

    抽哪一段？用长度比找「最配得上的那一段」：
      * 若 n=2,m=1：分别算 en[0]↔zh[0] 与 en[1]↔zh[0] 的比值，
        取**更接近 1.0 的那一段**留下配对，另一段摘出成 en-only 对。
      * 若 n=1,m=2：对称。
    两条硬门（与 `_split_wide_pairs` 同源，宁可不拆）：
      ① 参与配对的段不能是代码块/列表项；
      ② 留下的那对长度比必须在 [r_lo, r_hi] 内，**且**摘出的那段与
         对方唯一段的比值必须**明显更差**（否则说明两段都配得上，
         无法判定，保留原组）。
    判不下 → 原样返回单元素列表（调用方直接 extend，行为不变）。
    """
    n, m = len(p.en or []), len(p.zh or [])
    if en_paras is None or zh_paras is None:
        return [p]

    # 归一化成「多段在 en、单段在 zh」的视角
    flipped = (n == 1 and m >= 2)
    if flipped:
        b_multi, b_single = p.zh, p.en           # 索引在各自 paras 数组里
        multi_paras, single_paras = zh_paras, en_paras
    else:
        b_multi, b_single = p.en, p.zh
        multi_paras, single_paras = en_paras, zh_paras

    if len(b_multi) < 2 or len(b_single) != 1:
        return [p]

    sb = b_single[0]
    s_blk = single_paras[sb]
    # ① 单段侧是代码/列表 → 不拆（技术书里多段代码配一段中文是正常合并）
    if getattr(s_blk, "type", "") in ("code", "li", "pre") \
            or getattr(s_blk, "in_list", False):
        return [p]

    ratios = []
    for j in b_multi:
        mb = multi_paras[j]
        if getattr(mb, "type", "") in ("code", "li", "pre") \
                or getattr(mb, "in_list", False):
            ratios.append(None)                  # 代码段不参与竞争
            continue
        # ⚠ `_ratio(en_b, zh_b)` 的**第一个参数是英文、第二个是中文**。
        #   归一化后：flipped=True 时 multi 是中文（mb=中, s_blk=英），
        #   flipped=False 时 multi 是英文（mb=英, s_blk=中）。
        #   （2026-09-19 踩过：两个分支的入参写反 → 恒返回 0.0，
        #     `_depair_singleton` 静默失效，用户截图的 ch6 错配没被修好。）
        if flipped:
            r = _ratio(s_blk, mb)                # s_blk=英, mb=中
        else:
            r = _ratio(mb, s_blk)                # mb=英,  s_blk=中
        ratios.append(r)

    cands = [(abs(r - 1.0), i) for i, r in enumerate(ratios) if r is not None]
    if not cands:
        return [p]
    cands.sort()
    ...
    # ② 留下的那对必须合法
    if not (r_lo <= best_r <= r_hi):
        return [p]
    # ③ 必须有**唯一赢家**：第二名若同样接近 1.0，说明判不出，别拆
    if len(cands) > 1 and abs(cands[1][0] - best_d) < 0.15:
        return [p]

    keep_i = best_i                               # ratios 里的位置
    keep_m = b_multi[keep_i]                      # ⚠ 段落索引，与位置不同一空间
    # ⚠ 2026-09-19 修：原写法 `rest_m = [j for j in b_multi if j != best_i]`
    #   把**段落索引 j** 与 **ratios 位置 best_i** 相比。当段落索引恰好
    #   等于 best_i 时会误删，其余情况则**一个都删不掉** → rest 变成整个
    #   原组 b_multi，成品里出现「英文段重复出现两次」（真数据实锤：
    #   ch6 的 multi=[2,3]、best_i=1 → rest=[2,3]，段2 既单独成对又跟着段3
    #   再出现一次）。必须按**位置**摘除。
    rest_m = [b_multi[i] for i in range(len(b_multi)) if i != keep_i]
    if not rest_m:
        return [p]

    if flipped:                                  # zh 多、en 单
        out = [A.Pair(en=[sb], zh=[keep_m])]
        rest = A.Pair(en=[], zh=rest_m)
    else:                                        # en 多、zh 单
        out = [A.Pair(en=[keep_m], zh=[sb])]
        rest = A.Pair(en=rest_m, zh=[])
    # 余组是「原文有、译文缺」→ 保留 mt 标记语义（与 _split_wide_pairs 一致）
    rest.mt = getattr(p, "mt", None)
    out.append(rest)
    return out
...
def process_chapter(en_blocks, zh_blocks, key="", llm=None,
    ...
    K = A.estimate_k([b for b in en_blocks
                      if b.is_para and not _is_visual(b)],
                     [b for b in zh_kept
                      if b.is_para and not _is_visual(b)])
    ...
    for ei, zi in sec_pairs:
        ...
        # ── 提示框裸标签带过去（§6.41）──────────────────────────────────
        # `align.Section.box_labels` 与 `SectionResult` 是**两个类** ——
        # 不显式搬过来，那 266 个 `<h6>Note</h6>` 会在渲染层无声消失
        # （实测 ch4 的 20 个全丢）。只收**英文侧**：中译本把提示框摊平成
        # 普通段落，本来就没有这个标签。
        _blx: list = []
        for _i in ei:
            _blx.extend(getattr(en_secs[_i], "box_labels", None) or [])
        sr.box_labels = _blx
        if _blx:
            sr.note = ((sr.note + "；" if sr.note else "")
                       + f"提示框标题 {len(_blx)} 个")
