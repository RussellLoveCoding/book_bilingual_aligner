...
class Section:
    ...
    # v6（2026-09-19 §6.41）：提示框裸标签（`<h6>Note</h6>` 降级而来）。
    # **不参与对齐**（中译本无对应物），但内容保留、由渲染层当提示框标题输出。
...
def _check_section_conservation(blocks, secs, strict: bool):
    ...
    ⚠ 2026-09-19（§6.41）：`box_label` 也是「非标题块」，但它不进 paras/
    visuals，而是进 `sec.box_labels` —— 三处加起来才等于输入，否则会误报失败。
    """
    if not strict:
        return
    src_n = sum(1 for b in blocks if b.is_para or b.type == "box_label")
    out_n = sum(len(s.paras) + len(s.visuals) + len(s.box_labels) for s in secs)
    if out_n != src_n:
        raise RuntimeError(
            f"段落守恒对账失败：输入非标题块 {src_n} 个，"
            f"切分后仅 {out_n} 个（paras+visuals+box_labels）。"
...
def _has_content(sec: Section) -> bool:
    ...
    """
    if sec.title or sec.visuals or sec.paras or sec.box_labels:
        return True
    return any(b.is_para or b.type == "box_label" for b in sec.blocks)


def _finish(sec: Section, blocks: list) -> Section:
    """把 blocks 拆成 paras + visuals，并记录图位相对段落的位置。

    ⚠ 2026-09-19（§6.41）：`box_label`（提示框裸标签，如 `<h6>Note</h6>`）
    **不进 paras** —— 中译本没有它的对应物，混进去就是孤儿 pair，或把上一条
    公式的残渣吸进中文格（`zh=，k` 配 `en=Note`）。它挂在 `sec.box_labels`
    上，由渲染层当提示框标题输出（内容不丢）。
    """
    sec.blocks = list(blocks)
    sec.paras, sec.visuals, sec.box_labels = [], [], []
    for b in blocks:
        if b.type == "heading":
            continue
        if b.type == "box_label":
            sec.box_labels.append(b)
            continue
        if _is_visual(b):
