...
class Block:
    ...
    def is_para(self) -> bool:
        ...
        2026-09-19（§6.41）新增：`box_label`（提示框裸标签，如 `<h6>Note</h6>`）
        **不是**正文段 —— 中译本里没有它的对应物，混进段落流就会变孤儿 pair，
        或把上一条公式的残渣吸进中文格。它由渲染层当提示框标题输出（内容不丢）。

        ⚠ 调用方若原来写 `b.type != "heading"`，改成 `b.is_para` 即可获得
        与旧代码**逐位相同**的行为（`is_para` 为真 ⟺ 非 heading 且非 box_label），
        唯一差别是 `box_label` 被正确排除。
        """
        return self.type not in ("heading", "box_label")
...
#
# ⚠ 2026-09-18 修：类名可能是 `h2a` / `h2b` —— 剑桥社转制时用**字母后缀**
# 区分「同层级但排版略有不同」的标题（`h2a` 是带引号/斜体的二级标题）。
# 旧正则 `(?:^|[\s_-])h([1-6])(?:[\s_-]|$)` 要求 `hN` 后必须紧跟分隔符或
# 串尾，`h2a` 的 `a` 两个条件都不满足 → **整条标题被当成正文段吞掉**。
# 实测《概率论沉思录》中招 5 处，每一处都直接毁掉其下整棵小节树：
#   13.12.1 ‘Objectivity’ of decision theory  →  13.12 的 5 个子节全体错位
#    7.27.1 Terminology again   ·   9.16.1 The irrationalists
#    4.8.1 Etymology            ·  19.7.1 A paradox
# 这正是用户点名的一批「对齐错配」（7.27 / 9.16 / 13.12 / 4.8）的**根因**：
# 英文侧少了一个小节标题，中文侧有 → 该子节起所有小节配对左偏一格。
# 层级取数字，字母只作变体标记（`h2a` 即 2 级）。
...
def _box_label_of(text: str, box: str) -> bool:
    """`text` 是不是提示框容器 `box` 的**裸标签**（`<h6>Note</h6>` 这种）。

    背景（2026-09-19 实测《机器学习实战》，见 docs/HANDOFF.md §6.41）
    ---------------------------------------------------------------
    英文原版（O'Reilly DocBook）把提示框写成：

        <div data-type="note" epub:type="note"><h6>Note</h6><p>正文…</p></div>
        <div data-type="warning" epub:type="warning"><h6>Warning</h6><p>…</p></div>

    `epubparse.py` 已有一条正确设计（见 `_NON_SECTION_*`）：容器内的 heading
    **不能**当小节标题，否则 ML 第 4 章被 Equation/Warning 标签切成 30 个假小节、
    bad 率 80%。但那条设计只是把它**降级成 `para`** —— 于是它进了段落流，
    DP 把它当可配对的正文段，而**中译本把提示框摊平成了普通段落、没有这个标签**
    （实测：中文版「只有标签、无正文」的独立段落数 = 0）⇒ 两种恶果：

      ① 判 1:0 变孤儿：`<div class="pair"><p class="en … boxed">Note</p></div>`
      ② DP 抓**上一条公式的残渣**填进中文格：
         `zh=<i>，</i><i>k</i>` 配 `en=Note`、`zh=23` 配 `en=Warning`

    判据（**纯结构信号，零词表** —— 铁律 11）
    -----------------------------
    ① 祖先链里有提示框容器（`box` 非空，来自 `data-type`/`class`）；
    ② 段文本**恰好等于**该容器的类型名（`fullmatch`，大小写不敏感）——
       不是前缀、不是包含：`"Note that …"` 这种正文**不命中**。

    ② 之所以能这么严格，是因为判据来自**容器自身声明的类型**，不是外部词表：
    容器说自己是 `note`，里面的 `<h6>` 又正好写着 `Note`，那就是它的标题。
    换本书、换个出版社同样成立，无需按书调参。

    ⚠ 覆盖范围：ML 英文原版 266 个提示框（tip 127 / note 81 / warning 57 /
    caution 1）**全部**是 `<hN>` 裸标签形态；prob / think2 / nexus 各 0 个
    ⇒ 本判据对既有的三本回归书**结构上不可能命中**（零回归风险）。
    """
    if not box:
        return False
    t = (text or "").strip()
    if not t or len(t) > 24:
        return False
    return t.lower() == box.strip().lower()
...
def parse_blocks(src: str, strict: bool = True,
    ...
    for m in TAG_RE.finditer(body_text):
        ...
        # ⚠ 必须在**降级分支之前**读 —— 降级分支现在要用 `_box` 判「裸标签」，
        # 而 frames 稍后会被消费掉（祖先链是原文容器的唯一遗存）。
        ...
        if btype == "heading" and (
            ...
            # ── 提示框裸标签：降级**但不进段落流**（2026-09-19 §6.41）─────
            # `<div data-type="note"><h6>Note</h6><p>正文</p></div>` 里的
            # `Note` 是容器的小标题，不是正文段。若仍降级成 `para`，DP 会把
            # 它当可配对段 → 中文侧没有对应物 → 孤儿 pair，或抓上一条公式的
            # 残渣填中文格（`zh=，k` 配 `en=Note`）。判据见 `_box_label_of`。
            btype = ("box_label" if _box_label_of(text, _box) else "para")
            level = 0
