...
# group 用来在侧边栏分组：普通使用者只看「上手指南」，技术细节折叠在下面。
DOCS: list[tuple[str, str, str, str]] = [
    # (分组, 标题, 相对路径, 一句话说明)
    ("guide", "上手指南",   "docs/上手指南.md",  "零基础也能用：点几步就出书"),
    ("tech",  "项目总览",   "README.md",                    "这是什么、怎么开始"),
    ("tech",  "使用说明",   "docs/使用说明.md",              "怎么装、怎么跑、常见问题"),
    ("tech",  "工具说明",   "tools/README.md",               "模块划分与流水线细节"),
    ("tech",  "技术设计",   "bilingual-epub-design-v2.md",   "22 节设计文档"),
    ("tech",  "交接单",     "docs/HANDOFF.md",               "给接手人的：当前指标与已知缺陷"),
]

_GROUP_LABEL = {"guide": "上手指南", "tech": "技术文档"} 


def _docs_available() -> list[dict]:
    """返回实际存在的文档（没写的不显示），按分组排好。"""
    by_group: dict[str, list] = {}
    for group, title, rel, desc in DOCS:
        p = ROOT / rel
        if p.exists():
            by_group.setdefault(group, []).append(
                {"title": title, "path": rel, "desc": desc,
                 "size": p.stat().st_size})
    out = []
    for g in ("guide", "tech"):
        for d in by_group.get(g, []):
            d["group"] = g
            out.append(d)
    return out
...
<style>
  ...
  *{box-sizing:border-box}
  ...
  .nsec-doc{padding-top:10px;color:#7f8b9c}
  .nsec-doc + .nav i{filter:none}
  ...
  /* ── 试对齐的免责提示（醒目但不吓人） ── */
  .warnbox{display:flex;gap:11px;align-items:flex-start;
      background:#2a2412;border:1px solid #6b5a1e;border-radius:9px;
      padding:12px 15px;margin:14px 0 0}
  .warnbox .ic{flex:0 0 auto;font-size:15px;line-height:1.5}
  .warnbox .tx{font-size:12.5px;line-height:1.65;color:#e3d9b8}
  .warnbox .tx b{color:#f0c352}
...
<aside class="side" id="side">
  ...
  <div class="navwrap">
    ...
    <a class="nav" data-view="demo"><i>⇄</i><span>试对齐<small>粘贴中英段落 · 实时匹配（不准，仅供示范）</small></span></a>
...
<div id="demoView" class="hide">
  ...
  <div class="warnbox">
    <div class="ic">⚠️</div>
    <div class="tx">
      <b>这里的对齐结果不准，仅供感受流程与格式，不要拿它判断成品质量。</b><br>
      本页只喂了「纯段落清单」，<b>没有经过完整流水线</b>：不做章节映射、不识别标题/题注/
      公式图、不做勘误与 LLM 裁决，也拿不到任何版面信息。因此它对
      <b>干净、无复杂排版的纯文本</b>（一整段对一整段、两侧段落数差不多）才比较有效；
      真实电子书里的<b>图表、公式、注释、小节合并、漏译</b>等情况，这里一律测不出来。<br>
      真正的成品质量请以「合成」页跑出来的 epub / HTML 为准。
    </div>
  </div>
...
def _render_page() -> str:
    """把侧边栏文档目录注入 PAGE（分「上手指南 / 技术文档」两组）。"""
    docs = _docs_available()
    rows: list[str] = []
    last_group = None
    for d in docs:
        g = d.get("group", "tech")
        if g != last_group:
            rows.append(f'<div class="nsec nsec-doc">{_GROUP_LABEL[g]}</div>')
            last_group = g
        rows.append(
            f'<a class="nav" data-doc="{_html.escape(d["path"], quote=True)}" '
            f'data-title="{_html.escape(d["title"], quote=True)}">'
            f'<i>{"🚀" if g == "guide" else "📄"}</i><span>{_html.escape(d["title"])}'
