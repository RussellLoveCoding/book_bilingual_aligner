...
def main() -> int:
    ...
    # ── 统一架构（BIL_ARCH=unified）下的判据翻转（2026-09-19）────────────
    # 统一架构的设计就是「英文骨架在前、译文作为兄弟节点紧跟其后」
    # （用户定调：沉浸式翻译式同构插入）。此时 `en 在前` **是期望行为**，
    # 不再是「顺序倒置」。真正要查的变成：**每个英文是否紧跟自己的中文**
    # —— 即「en 在前」的占比应≈100%，且不存在「英文后面没有中文」的长串。
    _uni = 'arch-unified' in html[:4000] or 'arch-unified' in html
    if _uni:
        print(f"\n★ 【统一架构】en 在前占比 {tot['en_first']/n:.2%}"
              f"（{tot['en_first']} / {n}）—— **这是设计预期，不是缺陷**"
              if n else "★ 【统一架构】无双侧 pair")
        print("  · 统一架构 = 英文原版骨架在前 + 译文紧随其后（沉浸式翻译式）")
        print("  · 判据翻转：查「英文后是否紧跟中文」，而非「中文是否在前」")
        print("  · 版本：`BIL_ARCH=unified`；legacy 请勿套用本条")
    elif n:
